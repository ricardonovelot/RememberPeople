...
//Ideas:
// - Open Keyboard inmediatly and select name
// - How to delete tags?, edit tags names
// - crop image
// maybe a click on a icon of the tag checks it and a click on the name of the tag opens the tag and the avillity to pin it
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        ...
                        ForEach(sortedTags) { tag in
                            Button("\(tag.name ?? "") (\(tagUsageCount[tag] ?? 0))") {
                                selectedTag = tag
                            }
                        }
                    } label: {
